// Vercel serverless webhook for Stripe payment events (safegrid)
...
const MAIL_FROM = process.env.SAFEGRID_MAIL_FROM || 'gentledesk632@agentmail.to';
const AGENTMAIL_KEY = process.env.AGENTMAIL_API_KEY || '';
const BUYERS_FILE = 'buyers.json';
const USERS_FILE = 'users.json';
const ACCESS_PEPPER = process.env.ACCESS_PEPPER || 'safegrid-pepper-xj6essw0';
...
async function sendEmail(buyer, code) {
  ...
  const body =
`Welcome to Safegrid™ — The Agent Guardrail System ✅

Hi ${buyer.name || 'there'},

Thank you! Your order is confirmed.

YOUR PERSONAL ACCESS CODE: ${code || 'SG-XXXXX-XXXXX'}

OPEN YOUR DOWNLOADS:
→ https://safegrid.vercel.app/download.html
(enter your email + access code above)

WHAT'S INSIDE (8 deliverables):
• 1. The Agent Guardrail System (core guide: the SCOPE→ARM→FENCE→EVALUATE loop)
• 2. The Agent Scope Card + Trust-Tier Worksheet (one page per agent)
• 3. The Least-Privilege Credential Kit (scoped, revocable, brokered keys)
• 4. The Blast-Radius Blueprint (sandbox + kill switch + circuit breakers + idempotency)
• 5. The Two-Key Reversibility Gate (gate the five SPEND letters, free everything else)
• 6. The Decision Ledger + Governance Tracker (Trust Score + drift watch)
...
START TONIGHT (the 30-minute Quick Start):
1. Pick ONE agent you run today and write its Scope Card (deliverable 2).
2. Classify its trust tier — be honest, most are T2 pretending to be T0.
3. Kill one credential it does not need, right now.
4. Add the kill switch — one file check before each action.

Most agents are missing all four guards. Put up one tonight and you have already won.

Your agent doesn't have to be dangerous to be autonomous.
...
const res = await fetch(`https://api.agentmail.to/v0/inboxes/${MAIL_FROM}/messages/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${AGENTMAIL_KEY}` },
    body: JSON.stringify({ to: [buyer.email], subject: "Your Safegrid™ Agent Guardrail System access ✅", text: body }),
  });
  return res.ok;
}
